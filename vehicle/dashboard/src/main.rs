use chrono::Local;
use clap::Parser;
use common::subscribers::SubscriberTaskSpawner;
use common::topics::*;
use dashboard::LedManager;
use log::{error, trace};
use slint::*;
            ...
            move || {
                if let Some(main_window) = weak_window.upgrade() {
                    ...
                    if let Some(ref state) = model.lock_state {
                        is_lock = state.state == vehicle_msgs::state::State::Lock as i32;
...
async fn main() -> Result<(), slint::PlatformError> {
    ...
    let led_manager = Arc::new(dashboard::LedManager::default());
